package com.employeeTravel.main.domain;

import java.util.Objects;

public class BookingDetails {
	private int bookingId;
	private int requestId;
	private double travelCost;
	private String hotelName;
	private String checkIn;
	private String checkOut;
	private String departureTime;
	private String arrivalTime;

	public BookingDetails() {
		
	}

	public BookingDetails(int bookingId, int requestId, double travelCost, String hotelName, String checkIn,
			String checkOut, String departureTime, String arrivalTime) {
		super();
		this.bookingId = bookingId;
		this.requestId = requestId;
		this.travelCost = travelCost;
		this.hotelName = hotelName;
		this.checkIn = checkIn;
		this.checkOut = checkOut;
		this.departureTime = departureTime;
		this.arrivalTime = arrivalTime;
	}

	public int getBookingId() {
		return bookingId;
	}

	public void setBookingId(int bookingId) {
		this.bookingId = bookingId;
	}

	public int getRequestId() {
		return requestId;
	}

	public void setRequestId(int requestId) {
		this.requestId = requestId;
	}

	public double getTravelCost() {
		return travelCost;
	}

	public void setTravelCost(double travelCost) {
		this.travelCost = travelCost;
	}

	public String getHotelName() {
		return hotelName;
	}

	public void setHotelName(String hotelName) {
		this.hotelName = hotelName;
	}

	public String getCheckIn() {
		return checkIn;
	}

	public void setChekIn(String chekIn) {
		this.checkIn = chekIn;
	}

	public String getCheckOut() {
		return checkOut;
	}

	public void setChekOut(String checkOut) {
		this.checkOut = checkOut;
	}

	public String getDepartureTime() {
		return departureTime;
	}

	public void setDepartureTime(String departureTime) {
		this.departureTime = departureTime;
	}

	public String getArrivalTime() {
		return arrivalTime;
	}

	public void setArrivalTime(String arrivalTime) {
		this.arrivalTime = arrivalTime;
	}

	@Override
	public int hashCode() {
		return Objects.hash(arrivalTime, bookingId, checkIn, checkOut, departureTime, hotelName, requestId, travelCost);
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		BookingDetails other = (BookingDetails) obj;
		return Objects.equals(arrivalTime, other.arrivalTime) && bookingId == other.bookingId
				&& Objects.equals(checkIn, other.checkIn) && Objects.equals(checkOut, other.checkOut)
				&& Objects.equals(departureTime, other.departureTime) && Objects.equals(hotelName, other.hotelName)
				&& requestId == other.requestId
				&& Double.doubleToLongBits(travelCost) == Double.doubleToLongBits(other.travelCost);
	}

	@Override
	public String toString() {
		return "BookingDetails [bookingId=" + bookingId + ", requestId=" + requestId + ", travelCost=" + travelCost
				+ ", hotelName=" + hotelName + ", chekIn=" + checkIn + ", chekOut=" + checkOut + ", departureTime="
				+ departureTime + ", arrivalTime=" + arrivalTime + "]";
	}

}
